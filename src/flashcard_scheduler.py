...
import db
import flashcards
from config import FLASHCARD_DAILY_NEW_LIMIT, FLASHCARD_DAILY_REVIEW_LIMIT, FLASHCARD_LEVEL
from flashcard_handlers import format_card_front
...
async def send_daily_flashcards(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in users:
        ...
        try:
            stats = flashcards.get_flashcard_stats(telegram_user_id, FLASHCARD_LEVEL)
            ...
            card = None
            if stats["due"] > 0 and FLASHCARD_DAILY_REVIEW_LIMIT > 0:
                card = flashcards.pick_due_card(telegram_user_id, FLASHCARD_LEVEL)
            elif stats["new"] > 0 and FLASHCARD_DAILY_NEW_LIMIT > 0:
                card = flashcards.pick_new_card(telegram_user_id, FLASHCARD_LEVEL)
            if card:
                ...
                await context.bot.send_message(chat_id=chat_id, text=format_card_front(card))
